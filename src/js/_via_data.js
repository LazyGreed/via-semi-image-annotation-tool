  ...
  return new Promise(function(ok_callback, err_callback) {
    try {
      ...
      for (var i = 0; i < filelist.length; ++i) {
        ...
        this.store.file[fid] = new _via_file(fid, item.fname, _VIA_FILE_TYPE.IMAGE, item.loc, src, null, null);
  ...
  return new Promise(function(ok_callback, err_callback) {
    try {
      ...
      for (var i = 0; i < metadata_list.length; ++i) {
        ...
        var z_fp = _via_util_float_arr_to_fixed(m.z || [], _VIA_FLOAT_FIXED_POINT);
        var xy_fp = _via_util_float_arr_to_fixed(m.xy || [], _VIA_FLOAT_FIXED_POINT);
        this.store.metadata[mid] = new _via_metadata(m.vid, z_fp, xy_fp, m.av || {});
...
_via_data.prototype.metadata_update_xy = function(vid, mid, xy) {
  return new Promise(function(ok_callback, err_callback) {
    try {
      ...
      this.store.metadata[mid].xy = _via_util_float_arr_to_fixed(xy, _VIA_FLOAT_FIXED_POINT);
...
_via_data.prototype.metadata_delete_spatial_by_vid = function(vid) {
  return new Promise(function(ok_callback, err_callback) {
    try {
      ...
      var mid_list = this.cache.mid_list.hasOwnProperty(vid) ? this.cache.mid_list[vid].slice(0) : [];
      ...
      this.metadata_delete_bulk(vid, spatial_mid_list).then(function(ok) {
        ok_callback(ok);
      }, function(err) {
        err_callback(err);
      });
  ...
  return new Promise(function(ok_callback, err_callback) {
    try {
      if (blob_data && typeof blob_data.arrayBuffer === 'function') {
        blob_data.arrayBuffer().then(function(buffer) {
...
_via_data.prototype._project_get_file_blob = function(fid) {
  return new Promise(function(ok_callback, err_callback) {
    ...
    _via_util_fetch_blob(file_src).then(function(image_blob) {
...
_via_data.prototype.project_save = function() {
  return new Promise(function(ok_callback, err_callback) {
    ...
    for (var i = 0; i < fid_list.length; ++i) {
      (function(fid, filename, self) {
        image_entry_promises.push(
          self._project_get_file_blob(fid).then(function(blob_data) {
            return self._blob_to_uint8(blob_data).then(function(data_uint8) {
              var archive_name = self._project_archive_entry_name(fid, filename);
              export_store._via_image_archive.image_fid_to_path[fid] = archive_name;
              return { name: archive_name, data: data_uint8 };
            });
          }, function(err) {
            missing_images.push({ fid: fid, fname: filename, reason: err.toString() });
            return null;
          })
        );
    ...
    Promise.all(image_entry_promises).then(function(image_entries) {
      var zip_entries = [];
      zip_entries.push({
        name: 'project.json',
        data: _via_util_text_to_uint8(JSON.stringify(export_store, null, 2)),
      });
      ...
      _via_util_zip_create_blob(zip_entries).then(function(zip_blob) {
        var filename = ['via_project_', _via_util_date_to_filename_str(Date.now()), '.zip'].join('');
...
_via_data.prototype.project_load = function(project_data_str) {
  return new Promise(function(ok_callback, err_callback) {
    try {
      ...
      this.project_load_json(project_json_data).then(function() {
        ok_callback();
      }, function(err) {
        err_callback(err);
      });
...
_via_data.prototype.project_load_file = function(project_file) {
  return new Promise(function(ok_callback, err_callback) {
    ...
    if (file_ext === 'zip' || project_file.type === 'application/zip') {
      this.project_load_zip_file(project_file).then(function(ok) {
        ok_callback(ok);
      }, function(err) {
        err_callback(err);
      });
    ...
    _via_util_load_text_file(project_file, function(project_data_str) {
      this.project_load(project_data_str).then(function() {
        ok_callback({
...
_via_data.prototype.project_load_zip_file = function(project_zip_file) {
  return new Promise(function(ok_callback, err_callback) {
    _via_util_zip_extract(project_zip_file).then(function(zip_entries) {
      try {
        ...
        var project_json_str = _via_util_uint8_to_text(zip_entries[project_entry_name]);
        var project_json_data = JSON.parse(project_json_str);

        var archive_map = {};
        if (project_json_data.hasOwnProperty('_via_image_archive') &&
            project_json_data._via_image_archive &&
            project_json_data._via_image_archive.hasOwnProperty('image_fid_to_path')) {
          archive_map = project_json_data._via_image_archive.image_fid_to_path;
        }
        ...
        for (var fid in project_json_data.file) {
          ...
          if (archive_map.hasOwnProperty(fid)) {
            ...
            archive_name = this._project_archive_entry_name(fid, file_record.fname);
          }

          if (!zip_entries.hasOwnProperty(archive_name)) {
            archive_name = '';
            for (var zip_name in zip_entries) {
              if (_via_util_get_filename_from_uri(zip_name) === file_record.fname) {
          ...
          if (archive_name === '' || !zip_entries.hasOwnProperty(archive_name)) {
            continue;
          }
        ...
        this.project_load_json(project_json_data, file_ref_map).then(function() {
          ok_callback({
...
_via_data.prototype.project_load_json = function(project_json_data, file_ref_map) {
  return new Promise(function(ok_callback, err_callback) {
    try {
      this._release_all_file_resources();
      this.store = project_json_data;
      if (!this.store.config) {
        this.store.config = { file: { loc_prefix: { '1': '', '2': '', '3': '', '4': '' } }, ocr: { endpoint: _VIA_CONFIG.OCR_ENDPOINT, timeout_ms: _VIA_CONFIG.OCR_TIMEOUT_MS }, ui: {} };
      }
      ...
      if (!this.store.config.ocr) {
        this.store.config.ocr = { endpoint: _VIA_CONFIG.OCR_ENDPOINT, timeout_ms: _VIA_CONFIG.OCR_TIMEOUT_MS };
      }
  ...
  return new Promise(function(ok_callback, err_callback) {
    try {
      ...
      for (var i = 0; i < this.store.project.vid_list.length; ++i) {
        ...
        var mid_list = this.cache.mid_list.hasOwnProperty(vid) ? this.cache.mid_list[vid] : [];
        for (var m = 0; m < mid_list.length; ++m) {
          ...
          image_entry.boxes.push(_via_util_normalize_quad_points(md.xy.slice(1)));
        }

        out.images.push(image_entry);
      }

      var data_blob = new Blob([JSON.stringify(out, null, 2)], { type: 'application/json;charset=utf-8' });
      var filename = ['via_quad_export_', _via_util_date_to_filename_str(Date.now()), '.json'].join('');
