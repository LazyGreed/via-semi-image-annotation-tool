...
_via_view_annotator.prototype._show_start_info = function() {
  ...
  wrapper.innerHTML = '<h2>Image-Only Quadrilateral Annotator</h2>' +
    '<p>Add images from the toolbar, then click to define 4 points for each quadrilateral.</p>' +
...
_via_view_annotator.prototype._view_annotate_single_image = function(vid) {
  ...
  this.file_annotator = new _via_file_annotator(this, this.d, vid, '', file_container);
  this.file_annotator._file_load().then(function() {
    _via_util_msg_show('Image ready. Click 4 points to create a quadrilateral.');
  }, function(err) {
    console.warn(err);
...
_via_view_annotator.prototype.set_region_draw_shape = function(shape) {
  ...
  _via_util_msg_show('Quadrilateral mode enabled. Click 4 points to create one box.');
};
