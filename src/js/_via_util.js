...
function _via_util_fetch_blob(uri, timeout_ms) {
  var timeout = typeof timeout_ms === 'number' ? timeout_ms : _VIA_CONFIG.OCR_TIMEOUT_MS;
  return new Promise(function(ok_callback, err_callback) {
    ...
    fetch(uri, { signal: controller.signal }).then(function(resp) {
      if (!resp.ok) {
...
function _via_util_ocr_detect(image_blob) {
  return new Promise(function(ok_callback, err_callback) {
    ...
    fetch(_VIA_CONFIG.OCR_ENDPOINT + '/detect', {
      ...
      if (!resp.ok) {
        return resp.json().then(function(err_payload) {
          var detail = '';
          if (err_payload && typeof err_payload.error === 'string' && err_payload.error.length) {
            detail = ': ' + err_payload.error;
          }
          throw new Error('OCR server returned HTTP ' + resp.status + detail);
        }).catch(function() {
...
function _via_util_uint8_to_file(data, filename, mime_type) {
  var blob = new Blob([data], { type: mime_type || _via_util_infer_mime_type_from_filename(filename) });
...
function _via_util_zip_extract(zip_file_or_buffer) {
  return new Promise(function(ok_callback, err_callback) {
    ...
    } else if (zip_file_or_buffer && typeof zip_file_or_buffer.arrayBuffer === 'function') {
    ...
    read_promise.then(function(buffer) {
      try {
        ...
        while (offset + 4 <= bytes.length) {
          ...
          if (signature === 0x04034b50) {
            ...
            var entry_name = decoder.decode(bytes.slice(name_start, name_end));
