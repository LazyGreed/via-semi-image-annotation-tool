...
_via_view_manager.prototype._view_selector_option_html = function(vindex, vid) {
  ...
  oi.innerHTML = '[' + (parseInt(vindex, 10) + 1) + '] ' + decodeURI(this.d.store.file[fid].fname);
  return oi;
};
...
_via_view_manager.prototype._view_selector_update = function() {
  var existing_vid = '';
  if (this.view_selector.selectedIndex !== -1 && this.view_selector.options.length) {
    existing_vid = this.view_selector.options[this.view_selector.selectedIndex].value;
  }

  this._view_selector_clear();

  for (var vindex = 0; vindex < this.d.store.project.vid_list.length; ++vindex) {
...
_via_view_manager.prototype._on_add_media_local = function() {
  _via_util_file_select_local(_VIA_FILE_SELECT_TYPE.IMAGE, this._file_add_local.bind(this), true);
};
...
_via_view_manager.prototype._on_next_view = function() {
  if (!this.view_selector.options.length || this.view_selector.selectedIndex === -1) {
    return;
  }
